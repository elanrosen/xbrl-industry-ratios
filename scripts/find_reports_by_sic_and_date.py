...
def main(input_csv, sic_json, output_json):
    # Load the spinoff ticker to SIC code mapping
    with open(sic_json, 'r') as file:
        sic_code_mapping = json.load(file)

    # Load tickers CSV
    tickers_df = pd.read_csv(input_csv, parse_dates=['Date'])
    ...
    print(f"Operation completed, data saved to '{output_json}'.")

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python report_id_finder.py <input_csv_file> <input_sic_json_file> <output_json_file>")
        sys.exit(1)
    input_csv_file = sys.argv[1]
    input_sic_json_file = sys.argv[2]
    output_json_file = sys.argv[3]
    main(input_csv_file, input_sic_json_file, output_json_file)
